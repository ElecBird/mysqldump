        ...
        createSchemaDump:['getTables',function(callback,results){
            ...
            async.parallel(run,function(err,data){
                ...
                for(var i in data){
                    ...
                    if(r.indexOf('CONSTRAINT') !== -1){
                        var constraints = 'ALTER TABLE ' + //Alter Table
                            r.substring((r.indexOf('CREATE TABLE IF NOT EXISTS ') + 'CREATE TABLE IF NOT EXISTS '.length), r.indexOf(' (')) // table name
                            + '\n' +
                            r.substring(r.indexOf('CONSTRAINT'), r.indexOf(') ENGINE'));
                        constraints = constraints.replace(new RegExp('CONSTRAINT', 'g'), 'ADD CONSTRAINT'); // add ADD to all constraints
                        r = r.replace(r.substring(r.indexOf('CONSTRAINT'), r.indexOf(') ENGINE')), ''); // Remove the constraint
                        r = r.replace(',\n  ) ENGINE', '\n) ENGINE'); // Remove commas that were before the constraints
                        if(constraintList.indexOf('undefined') === -1) constraintList += constraints;
                    }
                    r = r.replace('undefined;', ''); // Remove the undefined parts
                    resp.push(r)
